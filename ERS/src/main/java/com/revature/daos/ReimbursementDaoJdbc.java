...
public class ReimbursementDaoJdbc implements ReimbursementDao {
	...
	public int updateReimb(Reimbursement r) {
	    ...
	    LocalDateTime submittedDateTime = LocalDateTime.parse(subTime, formatter);
		try(Connection conn = ConnectionUtil.getConnection()){
			PreparedStatement ps = conn.prepareStatement("UPDATE ers_reimbursement SET reimb_amount = ? , reimb_submitted = ? , "+
					"reimb_description= ? , reimb_receipt = ? , reimb_type_id = ? "
					+ "WHERE reimb_id = ?");
			ps.setInt(1, r.getAmount());
			ps.setObject(2, submittedDateTime);
			ps.setString(3, r.getDescription());;
			ps.setString(4, r.getReceipt());
	...
	public int updateStatus(Reimbursement r) {
		DateTimeFormatter formatter= DateTimeFormatter.ofPattern("uuuu-MM-dd HH:mm:ss");
	    String resTime = r.getResolved();
	    LocalDateTime resDateTime = LocalDateTime.parse(resTime, formatter);
		try(Connection conn = ConnectionUtil.getConnection()){
			PreparedStatement ps = conn.prepareStatement("UPDATE ers_reimbursement SET reimb_status_id = ? , reimb_resolved = ? "
					+ "WHERE reimb_id = ?");
			ps.setInt(1, r.getReimbStatusId());
			ps.setObject(2, resDateTime);
			ps.setInt(3, r.getId());
			ps.executeUpdate();
		}
		catch(SQLException e) {
